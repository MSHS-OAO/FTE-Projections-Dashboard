...
folderOracle <- paste0("/SharedDrive/deans/Presidents/SixSigma/MSHS Productivity/",
                       "Productivity/Universal Data/Labor/Raw Data/MSHQ Oracle/")
#List files from MSHQ Raw folder
Oracle_file_list <- list.files(path=folderOracle, pattern="*.txt")
details <- file.info(list.files(path = folderOracle, pattern="*.txt", full.names = T)) %>%
...
data_msh_msq_oracle <- readRDS(paste0("/SharedDrive/deans/Presidents/SixSigma/MSHS Productivity/",
                                      "Productivity/Universal Data/Labor/RDS/data_MSH_MSQ_oracle.rds"))

# get the min start date and max end date associated with each file in rds
dates_check <- data_msh_msq_oracle %>%
...
for (i in 1:length(ORACLElist)) {
  write.table(ORACLElist[[i]],
              file = paste0("/SharedDrive/deans/Presidents/SixSigma/Individual Folders/Current Employees/Engineers/Greg Lenane/MSHQ Raw/",
                            i,"_MSHQ_Raw.txt"),
              row.names = F, sep = "~")
}
